...
export function GameAction({ gameState, onNextStep, onBid, onTricks, onReplayRound }) {
  ...
  if (phase === "tricks") {
    ...
    const allPlayersData = playerOrder.map((playerIdx, bidIndex) => ({
      ...
      isCurrent: bidIndex === currentTrickIndex,
      isEntered: bidIndex < currentTrickIndex
    }))
      ...
      <Card>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="space-y-2 text-3xl font-bold text-primary">{players[currentPlayerIndex]}</div>
          <TrickInput
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        {Array.from({ length: maxTricks + 1 }, (_, i) => i).map((trickValue) => {
